...
public class AuthService
{
			...
			@Value("${jwt.secret}") String jwtSecret) 
	{
		...
		{
			throw new IllegalArgumentException("JWT_SECRET in application.propertis must be at least 64 bytes long for HS512.");
		}
	...
	public boolean validateToken(String token) {
		try {
            // Parse token, verify signature and expiration
            Jwts.parserBuilder()
	...
	public void logout(User authenticatedUser) {
	    if(authenticatedUser == null) return; // prevents NullPointerException
	    JWTToken token = jwtTokenRepository.findByUserId(authenticatedUser.getUserId());
	    if(token != null) {
	        jwtTokenRepository.delete(token); // deletes token entity
	    }
	}
